...
# --model_name_or_path output/chinese-roberta-wwm-ext/mc/model 
# --tokenizer_name output/chinese-roberta-wwm-ext/mc/model 
# --pad_to_max_length
# --max_seq_length 512 
# --per_device_eval_batch_size 100
# --output_dir output\chinese-roberta-wwm-ext\mc
# --debug
...
def main():
    ...
    result = None
    model.eval()
    for batch in tqdm(test_dataloader): # batch type is dict
        # ipdb.set_trace()
        for k in batch.keys():
            batch[k] = batch[k].to(device) # input_ids, attention_mask, 
        with torch.no_grad():
            outputs = model(**batch)
        # (batch, num_classes)
        predictions = outputs.logits.argmax(dim=-1) # compared in num_classes dimension
        predictions = predictions.cpu()
        result = predictions if result is None else torch.cat((result, predictions))
    ...
    for i in range(len(result)):
        ...
        # print(result_json[i]['paragraphs'][int(result_numpy[i])])
        result_json[i]["relevant"] = result_json[i]['paragraphs'][int(result_numpy[i])]

    if not args.debug:
        inference_path = os.path.join(args.output_dir, "test.json")
        with open(inference_path, 'w', encoding = 'utf-8') as file:
